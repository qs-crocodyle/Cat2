...
local function CreateRulesWindow()
    ...
    heading:SetText(Cat2.L("Cat2 使用规则"))
    ...
    AddSection(rulesWindow, Cat2.L("流程顺序"), Cat2.L("流程从上向下执行。拖动左侧卡片可以调整顺序。\n部分卡片成功执行后会停止本轮流程，重要卡片请放在合适位置。"), -66)
    AddSeparator(rulesWindow, -130)
    AddSection(rulesWindow, Cat2.L("卡片状态"), Cat2.L("点击左侧卡片后，可以暂停、恢复、隐藏或删除。\n被动卡片|cffb880f0（紫色）|r会影响整个流程；暂停后，它的被动效果不会生效。"), -146)
    AddSeparator(rulesWindow, -210)
    AddSection(rulesWindow, Cat2.L("快捷小窗"), Cat2.L("显示在小窗中的卡片可以随时点击暂停或恢复。\n主界面与快捷小窗相互独立，关闭主界面不会关闭小窗。"), -226)
    AddSeparator(rulesWindow, -290)
    AddSection(rulesWindow, Cat2.L("配置使用"), Cat2.L("输入 |cffff4fa3/cat2 配置名|r 执行对应流程。配置按角色保存。\n导入配置时会检查职业；同名配置需要确认后才能覆盖。"), -306)

    local closeButton = CreateRuleButton(rulesWindow, Cat2.L("关闭"), 96)
