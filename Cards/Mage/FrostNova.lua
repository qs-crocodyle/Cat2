...
local card = {
    id = "mage_frost_nova",
    name = "冰霜新星",
    description = "有效距离内，冷却后施放|cff6bc7e0{spellRank}级|r冰霜新星",
    details = "有效距离内，冷却后按卡片设定等级施放冰霜新星；未学习指定等级时，直接施放冰霜新星并由游戏选择最高已学习等级。需要存在有效目标。会检查目标距离。仅在技能可用时尝试执行。成功执行时会阻断本轮后续卡片。",
    sort = 20,
    category = "class",
    classes = {
        MAGE = 3,
...
function card.Execute(context, step)
    ...
    if Cat2.SpellReady("冰霜新星") then  -- 可以增加保护 not player.buff["冰霜速冻"]
        ...
        if Cat2.GetSpellID("冰霜新星", rankText)>0 then
            Cat2.Cast("冰霜新星(" .. rankText .. ")")
        else
