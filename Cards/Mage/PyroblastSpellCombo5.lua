-- 炎爆术（五层法术连击）技能卡片；执行机制与三层版本一致。
local card = {
    id = "mage_pyroblast_spell_combo_5",
    name = "炎爆术（五层法术连击）",
    description = "当5层法术连击时，施放炎爆术",
    details = "当5层法术连击时，施放炎爆术。需要存在有效目标；目标火焰免疫时不会施放。成功执行时会阻断本轮后续卡片。",
    sort = 55,
    category = "class",
    classes = {
        MAGE = 2,
...
function card.Execute(context)
    ...
    if GetTime()-Cat2.GetMageCastPyroblastTimer()>0 then
        ...
        if Cat2.GetBuffApplications("Interface\\Icons\\Ability_Mage_Firestarter")>=5 then
            Cat2.Cast("炎爆术")
            return true
        end
