...
local card = {
    ...
    name = "扫击",
    -- 卡片标题下方显示的简短说明。
    description = "造成流血并获得连击点，自动识别是否吃流血",
    -- 预留给后续详情面板或 Tooltip 的完整功能说明。
    details = "造成流血并获得连击点，自动识别是否吃流血。需要存在有效目标。会检查当前资源。成功执行时会阻断本轮后续卡片。",
...
function card.RefreshRuntimeData()
    -- 天赋
    rakePower = 40-Cat2.IsTalentLearned(2,1)
	...
	if count >2 then rakePower = rakePower-3 end

	if Cat2.CheckInventoryItemName(18,"凶猛神像") then rakePower = rakePower-3 end
end
