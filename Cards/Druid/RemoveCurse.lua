-- 解除诅咒：按照治疗指向被动卡提供的顺序扫描友方单位。
local card = {
    id = "druid_remove_curse",
    name = "解除诅咒",
    description = "根据|cffb87ff0[被动卡]|r规则，受诅咒时施放解除诅咒",
    details = "按照治疗指向被动卡提供的目标顺序扫描友方单位，发现诅咒效果时施放解除诅咒。会检查施法状态、公共冷却、距离与视野。",
    sort = 260,
    category = "class",
    classes = {
        DRUID = 3,
...
local RemoveCurseMaxLevel = 0
local RemoveCurseTargetDelay = {}

function card.RefreshRuntimeData()
    RemoveCurseMaxLevel = 1  --Cat2.GetHighestRankOfSpell("解除诅咒")
end
...
function card.Remove(unit, member, context)
    ...
    if targetName and RemoveCurseTargetDelay[targetName] and RemoveCurseTargetDelay[targetName] - GetTime() > 0 then
        return false
    end
    RemoveCurseTargetDelay[targetName] = GetTime() + 1.0

    if RemoveCurseMaxLevel > 0 then
        return Cat2.CastSpellWithoutTarget("解除诅咒", unit, 1)
    end
