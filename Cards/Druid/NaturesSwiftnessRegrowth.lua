-- 自然迅捷 愈合 技能卡片。
local card = {
    id = "druid_natures_swiftness_regrowth",
    name = "自然迅捷 愈合",
    description = "根据|cffb87ff0[被动卡]|r规则，血量<|cff6bc7e0{triggerPercent}%|r时施放",
    details = "根据|cffb87ff0[被动卡]|r规则，血量低于卡片设定值时施放自然迅捷，并接续愈合。未单独设置时使用默认值30%。需要存在有效目标。仅对友方存活目标生效。会检查战斗状态、距离、视野、相关生命值与技能可用性。",
    sort = 232,
    category = "class",
...
local function GetConfiguredRankRange(context)
    ...
    for _, followupStep in ipairs(steps) do
        if followupStep and followupStep.enabled ~= 0 and followupStep.id == "druid_regrowth" then
...
function card.Health(unit, member, context, triggerPercent)
    local followupCard = Cat2.CardRegistry and Cat2.CardRegistry.ById["druid_regrowth"]
