...
local card = {
    ...
    name = "撕扯（二星）",
    -- 卡片标题下方显示的简短说明。
    description = "消耗2连击点造成流血，自动识别是否吃流血",
    -- 预留给后续详情面板或 Tooltip 的完整功能说明。
    details = "消耗2连击点造成流血，自动识别是否吃流血。需要存在有效目标。会检查当前资源。成功执行时会阻断本轮后续卡片。",
...
function card.Execute(context)
    ...
    if (player.power>=30 or player.buff["节能施法"]) and not Cat2.GetRipDot() and player.targetCombo==2 then
        CastSpellByName("撕扯")
        return true
    end
