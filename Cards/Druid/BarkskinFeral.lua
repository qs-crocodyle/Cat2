...
local card = {
    ...
    name = "树皮术（野性）",
    -- 卡片标题下方显示的简短说明。
    description = "血量低于|cff6bc7e0{triggerPercent}%|r时开启树皮术（野性）",
    -- 预留给后续详情面板或 Tooltip 的完整功能说明。
    details = "血量低于卡片设定值时，开启树皮术（野性）以降低所受伤害。会检查战斗状态。仅在技能可用时尝试执行。成功执行时会阻断本轮后续卡片。",
    ...
    cooldown = { type = "spell", name = "树皮术（野性）" },
    optionSchema = {
        {
            key = "triggerPercent",
            type = "number",
...
function card.Execute(context, step)
    ...
    if player.percentHealth < triggerPercent and Cat2.SpellReady("树皮术（野性）") then
        Cat2.Cast("树皮术（野性）")
        return true
    end
