-- 自然迅捷 治疗之触 技能卡片。
local card = {
    id = "druid_natures_swiftness_healing_touch",
    name = "自然迅捷 治疗之触",
    description = "根据|cffb87ff0[被动卡]|r规则，血量<|cff6bc7e0{triggerPercent}%|r时施放",
    details = "根据|cffb87ff0[被动卡]|r规则，血量低于卡片设定值时施放自然迅捷，并接续治疗之触。未单独设置时使用默认值30%。需要存在有效目标。仅对友方存活目标生效。会检查战斗状态、距离、视野、相关生命值与技能可用性。",
    sort = 231,
    category = "class",
    exclusiveGroup = "druid_natures_swiftness_followup",
    classes = {
        DRUID = 3,
    },
    icons = {
...
local function GetConfiguredRankRange(context)
    ...
    for _, followupStep in ipairs(steps) do
        if followupStep and followupStep.enabled ~= 0 and followupStep.id == "druid_healing_touch_target" then
...
function card.Health(unit, member, context, triggerPercent)
    local followupCard = Cat2.CardRegistry and Cat2.CardRegistry.ById["druid_healing_touch_target"]
    ...
    local function ActivateNaturesSwiftness()
        if Cat2.GetChanneled() >= 0.08 or not Cat2.SpellReady("自然迅捷") then
            return false
        end
        Cat2.Cast("自然迅捷")
        return true
