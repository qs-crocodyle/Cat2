-- 剔骨（三星）技能卡片。
local card = {
    id = "rogue_eviscerate_3",
    name = "剔骨（三星）",
    description = "消耗3连击点施放剔骨，血腥等待|cff6bc7e0{bloodyWaitSeconds}秒|r",
    details = "消耗3连击点施放剔骨，若血腥气息即将结束，则等待。需要存在有效目标。成功执行时会阻断本轮后续卡片。",
    sort = 23,
    category = "class",
    classes = {
        ROGUE = 1,
...
function card.Execute(context, step)
    ...
    -- 在更高星级割裂卡时，优先等待血腥气息保护。
    if HasBloody > 0 then
        if context:IsCardActive("rogue_rupture_bloody_3")
    ...
    if player.power >= 30 and player.targetCombo == 3 then
        Cat2.Cast("剔骨")
        return true
    end
