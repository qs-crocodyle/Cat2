-- 剔骨（二星）技能卡片。
local card = {
    id = "rogue_eviscerate_2",
    name = "剔骨（二星）",
    description = "消耗2连击点施放剔骨，血腥等待|cff6bc7e0{bloodyWaitSeconds}秒|r",
    details = "消耗2连击点施放剔骨，若血腥气息即将结束，则等待。需要存在有效目标。成功执行时会阻断本轮后续卡片。",
    sort = 22,
    category = "class",
    classes = {
        ROGUE = 1,
...
function card.Execute(context, step)
    ...
    if HasBloody > 0 then
        if context:IsCardActive("rogue_rupture_bloody_2")
    ...
    if player.power >= 30 and player.targetCombo == 2 then
        Cat2.Cast("剔骨")
        return true
    end
