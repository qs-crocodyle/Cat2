...
local card = {
    ...
    name = "剔骨（四星）",
    description = "消耗4连击点施放剔骨，血腥等待|cff6bc7e0{bloodyWaitSeconds}秒|r",
    details = "消耗4连击点施放剔骨，若血腥气息即将结束，则等待。需要存在有效目标。成功执行时会阻断本轮后续卡片。",
    sort = 24,
    category = "class",
    classes = {
        ROGUE = 1,
...
function card.Execute(context, step)
    ...
    if HasBloody > 0 then
        if context:IsCardActive("rogue_rupture_bloody_4")
    ...
    if player.power >= 30 and player.targetCombo == 4 then
        Cat2.Cast("剔骨")
        return true
    end
