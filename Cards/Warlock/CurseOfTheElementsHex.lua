-- 元素诅咒（邪咒）技能卡片。
local card = {
    id = "warlock_curse_of_the_elements_hex",
    name = "元素诅咒（邪咒）",
    description = "保持并施放元素诅咒和痛苦诅咒",
    details = "保持并施放元素诅咒和痛苦诅咒。需要存在有效目标。成功执行时会阻断本轮后续卡片。",
    sort = 25,
...
function card.Execute(context)
    ...
    if not player.targetBuff["元素诅咒"] or not Cat2.GetCurseAgonyDot() then
        Cat2.CastWarlockDot(context, "元素诅咒")
        return true
    end
