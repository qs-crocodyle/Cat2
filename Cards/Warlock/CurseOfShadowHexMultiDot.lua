-- 暗影诅咒（邪咒，多线DOT）技能卡片。
local card = {
    id = "warlock_curse_of_shadow_hex_multi_dot",
    name = "暗影诅咒（邪咒，多线DOT）",
    description = "为附近没有痛苦诅咒的敌人施放暗影诅咒",
    details = "扫描暗影诅咒射程内的敌人，并对其中没有痛苦诅咒的目标无目标施放暗影诅咒。需要邪咒天赋和 SuperWoW；每轮最多选择一个敌人施放。",
    sort = 154,
    ...
    for guid, _ in pairs(enemyList) do
        if UnitExists(guid) and UnitCanAttack("player", guid) and not UnitIsDeadOrGhost(guid)
            ...
            if not hasCurseOfAgony then
                if Cat2.CastSpellWithoutTarget("暗影诅咒", guid) then
                    return true
                end
            end
        end
