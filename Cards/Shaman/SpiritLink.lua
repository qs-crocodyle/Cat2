...
local card = {
    ...
    name = "灵魂链接",
    description = "根据|cffb87ff0[被动卡]|r规则，血量<15%危急时施放灵魂链接",
    details = "根据|cffb87ff0[被动卡]|r规则，血量<15%危急时施放灵魂链接。需要存在有效目标。仅对可攻击目标生效。会检查战斗状态。会检查相关生命值。仅在技能可用时尝试执行。",
    sort = 40,
    category = "class",
    classes = {
        SHAMAN = 3,
...
function card.Health(unit, member, context)
    ...
    -- 先确保技能已学
    local SpiritLinkLevel = Cat2.GetHighestRankOfSpell("灵魂链接")
    if SpiritLinkLevel>0 then

        if Cat2.SpellReady("灵魂链接") then
            return Cat2.CastSpellWithoutTarget("灵魂链接", unit, 1)
        end
