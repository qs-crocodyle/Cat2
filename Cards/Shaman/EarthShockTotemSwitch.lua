-- 大地震击切换图腾占位卡；具体逻辑后续补充。
local card = {
    id = "shaman_earth_shock_totem_switch",
    name = "大地震击切换图腾",
    description = "大地震击时切换为|cff6bc7e0{totemName}|r",
    details = "原始大地震击卡冷却即将结束且当前处于公共冷却前半段时，尝试将圣物栏切换为所选图腾，不影响大地震击的其他分支。换装本身不会阻断流程；同一轮只处理第一张满足条件的图腾联动卡。参数可从菜单选择，也可手动输入其他名称；已经装备或背包中没有所选图腾时不会重复操作。作为被动规则，启用时影响当前流程。",
    sort = 83,
    behavior = "passive",
    unique = true,
    category = "class",
...
function card.Apply(context, step)
    context.parameters.shamanEarthShockTotem = context:GetStepOption(step, "totemName") or ""
end
