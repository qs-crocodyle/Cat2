-- 先祖迅捷 治疗波 技能卡片。
local card = {
    id = "shaman_ancestral_swiftness_healing_wave",
    name = "先祖迅捷 治疗波",
    description = "根据|cffb87ff0[被动卡]|r规则，血量<|cff6bc7e0{triggerPercent}%|r时施放",
    details = "根据|cffb87ff0[被动卡]|r规则，血量低于卡片设定值时施放先祖迅捷，并接续治疗波。未单独设置时使用默认值30%。需要存在有效目标。仅对友方存活目标生效。会检查战斗状态、距离、视野、相关生命值与技能可用性。",
    sort = 49,
    category = "class",
...
local function GetConfiguredRankRange(context)
    ...
    for _, followupStep in ipairs(steps) do
        if followupStep and followupStep.enabled ~= 0 and followupStep.id == "shaman_healing_wave" then
...
function card.Health(unit, member, context, triggerPercent)
    local followupCard = Cat2.CardRegistry and Cat2.CardRegistry.ById["shaman_healing_wave"]
