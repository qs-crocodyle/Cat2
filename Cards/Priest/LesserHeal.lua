...
local card = {
    ...
    details = "根据|cffb87ff0[被动卡]|r规则，血量低于卡片设定值时，在设定等级区间内自适配等级施放次级治疗术。默认触发血量为99%。需要存在有效目标。仅对可攻击目标生效。会检查相关生命值。",
    sort = 10,
    category = "class",
    classes = {
        PRIEST = 2,
    ...
    optionSchema = {
        ...
        { key = "minimumRank", type = "number", label = "最小等级", default = 1, minimum = 1, maximum = 3 },
        { key = "maximumRank", type = "number", label = "最大等级", default = 3, minimum = 1, maximum = 3 },
    },
}

-- 次级治疗术
local PriestLesserHeal = {}
local PriestLesserHealEffect = {}
local PriestLesserHealFactor = 0.2
local PriestLesserHealMaxLevel = 3
...
function card.RefreshRuntimeData()
    ...
    PriestLesserHeal[1] = 30 * GreaterHeal
    PriestLesserHealEffect[1] = 46+(HealingPower*PriestLesserHealFactor)
    PriestLesserHeal[2] = 45 * GreaterHeal
    PriestLesserHealEffect[2] = 89+(HealingPower*PriestLesserHealFactor)
    PriestLesserHeal[3] = 75 * GreaterHeal
    PriestLesserHealEffect[3] = 154+(HealingPower*PriestLesserHealFactor)

    PriestLesserHealMaxLevel = Cat2.GetHighestRankOfSpell("次级治疗术")
end
...
function card.Health(unit, member, context, triggerPercent, minimumRank, maximumRank)
    ...
    -- 先确保技能已学
    if PriestLesserHealMaxLevel>0 then

        for i = maximumRank, minimumRank, -1 do
            if PriestLesserHealEffect[i] < HealthDec and Cat2.PlayerInformation.temporary.mana >= PriestLesserHeal[i] then
                return Cat2.CastSpellWithoutTarget("次级治疗术(等级 "..i..")", unit, 1)
            end
        end
        if Cat2.PlayerInformation.temporary.mana >= PriestLesserHeal[minimumRank] then
            return Cat2.CastSpellWithoutTarget("次级治疗术(等级 "..minimumRank..")", unit, 1)
        end
...
function card.Execute(context, step)
    ...
    if maximumRank > PriestLesserHealMaxLevel then maximumRank = PriestLesserHealMaxLevel end
    if minimumRank > PriestLesserHealMaxLevel then minimumRank = PriestLesserHealMaxLevel end
