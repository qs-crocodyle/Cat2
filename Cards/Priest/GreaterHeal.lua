...
local card = {
    ...
    details = "根据|cffb87ff0[被动卡]|r规则，血量低于卡片设定值时，在设定等级区间内自适配等级施放强效治疗术。默认触发血量为99%。需要存在有效目标。仅对可攻击目标生效。会检查相关生命值。",
    sort = 40,
    category = "class",
    classes = {
        PRIEST = 2,
    ...
    optionSchema = {
        ...
        { key = "minimumRank", type = "number", label = "最小等级", default = 1, minimum = 1, maximum = 5 },
        { key = "maximumRank", type = "number", label = "最大等级", default = 5, minimum = 1, maximum = 5 },
    },
}
...
function card.Health(unit, member, context, triggerPercent, minimumRank, maximumRank)
    ...
    if PriestGreaterHealMaxLevel>0 then
        ...
        for i = maximumRank, minimumRank, -1 do
            if PriestGreaterHealEffect[i] < HealthDec and Cat2.PlayerInformation.temporary.mana >= PriestGreaterHealMana[i] then
                return Cat2.CastSpellWithoutTarget("强效治疗术(等级 "..i..")", unit, 1)
            end
        end
        if Cat2.PlayerInformation.temporary.mana >= PriestGreaterHealMana[minimumRank] then
            return Cat2.CastSpellWithoutTarget("强效治疗术(等级 "..minimumRank..")", unit, 1)
        end
...
function card.Execute(context, step)
    ...
    local maximumRank = context:GetStepOption(step, "maximumRank") or 5
    if maximumRank > PriestGreaterHealMaxLevel then maximumRank = PriestGreaterHealMaxLevel end
    if minimumRank > PriestGreaterHealMaxLevel then minimumRank = PriestGreaterHealMaxLevel end
