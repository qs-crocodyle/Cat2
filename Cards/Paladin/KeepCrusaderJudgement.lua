-- 圣骑士惩戒系普通卡：保持目标身上的十字军审判。
local card = {
    id = "paladin_keep_crusader_judgement",
    name = "保持目标 十字军审判",
    description = "保持目标身上的十字军审判",
    details = "保持目标身上的十字军审判。需要存在有效目标。会检查目标距离。仅在技能可用时尝试执行。成功执行时会阻断本轮后续卡片。",
    sort = 3,
    unique = true,
...
function card.Execute(context)
    ...
    if player.targetBuff["十字军审判"] then
        return false
    end
