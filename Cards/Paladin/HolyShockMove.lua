-- 神圣震击（治疗，移动时）技能卡片。
local card = {
    id = "paladin_holy_shock_move",
    name = "神圣震击（治疗，移动时）",
    description = "移动时，根据|cffb87ff0[被动卡]|r规则，血量低于|cff6bc7e0{triggerPercent}%|r时施放神圣震击",
    details = "移动时，根据|cffb87ff0[被动卡]|r规则，血量低于卡片设定值时施放神圣震击。未单独设置时使用默认值90%。需要存在有效目标。仅对可攻击目标生效。会检查相关生命值与移动状态。仅在技能可用时尝试执行。成功执行时会阻断本轮后续卡片。",
    sort = 30.5,
    category = "class",
    classes = {
        PALADIN = 1,
...
local HolyShockLevel = 0

function card.RefreshRuntimeData()
    HolyShockLevel = Cat2.GetHighestRankOfSpell("神圣震击")
end
...
function card.Health(unit, member, context, triggerPercent)
    ...
    if HolyShockLevel>0 then
        ...
        if Cat2.SpellReady("神圣震击") then
            return Cat2.CastSpellWithoutTarget("神圣震击", unit, 1)
        end
...
function card.Execute(context, step)
    ...
    local triggerPercent = context:GetStepOption(step, "triggerPercent") or 90
