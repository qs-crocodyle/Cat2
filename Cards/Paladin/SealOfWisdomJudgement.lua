-- 智慧圣印 技能卡片。
local card = {
    id = "paladin_seal_of_wisdom_judgement",
    name = "保持智慧圣印/审判",
    description = "保持智慧圣印的同时，施放审判进行攻击",
    details = "保持智慧圣印的同时，施放审判进行攻击。需要存在有效目标。会检查目标距离。仅在技能可用时尝试执行。成功执行时会阻断本轮后续卡片。",
    sort = 20.1,
    category = "class",
...
function card.Execute(context)
    ...
    if not Cat2.Seal("智慧圣印") then
        Cat2.CastWithNampower("智慧圣印")
        return true
    else
